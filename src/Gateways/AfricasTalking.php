...
class AfricasTalking implements SmsGateway
{
	...
	public function sendPremium($options)
	{
		if (empty($options['to']) || empty($options['message'])) {
			return json_encode([ 'error' => true, 'message' => 'recipient and message must be defined' ]);
		}
	...
	public function createSubscription ($options)
	{
		if (empty($options['phoneNumber']) ||
            ...
            empty($options['checkoutToken'])) {
                return json_encode([
                    'error' => true,
                    'message' => "phoneNumber, shortCode keyword and checkoutToken must be specified",
                    ]);
		}
		...
		return Network::post($this->contentUrl.'subscription/create', ['form_params' => $data ], $this->defaultHeaders);
	}
	...
	public function deleteSubscription ($options)
	{
		if (empty($options['phoneNumber']) ||
            ...
            empty($options['keyword'])) {
			    return json_encode([
                'error' => true,
                'message' => 'phoneNumber, shortCode and keyword must be specified',
                ]);
		}
		...
		return Network::post($this->contentUrl.'subscription/delete', ['form_params' => $data ], $this->defaultHeaders);
    }
}
